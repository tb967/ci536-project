document.addEventListener("DOMContentLoaded", () => {
    // Update cart count in the header for all pages
    updateCartCount();
    
    // If on the products listing page, load items
    if (document.getElementById("itemList")) {
        loadItems();
    }
    
    // If on the cart page, load cart contents
    ...
    // Update cart count in the header - this function will work on all pages
    function updateCartCount() {
        const cart = JSON.parse(localStorage.getItem('cart')) || [];
        const totalItems = cart.reduce((total, item) => total + parseInt(item.quantity), 0);
        
        // Update all cart count elements with class 'cart-count'
